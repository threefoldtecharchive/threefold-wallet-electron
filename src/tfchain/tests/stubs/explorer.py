...
class TFChainExplorerGetClientStub:
    ...
    def explorer_get(self, endpoint):
        ...
        match = hash_template.match(endpoint)
        if match:
            return self.block_get(jsstr.to_int(match.group(1)))
        ...
        if match:
            return self.mint_condition_get(height=jsstr.to_int(match.group(1)), endpoint='/explorer/mintcondition/' + match.group(1))
    ...
    def explorer_post(self, endpoint, data):
        ...
        if match:
            ...
            return jsstr.sprintf('{"transactionid":"%s"}', transactionid.__str__())
        raise Exception("invalid endpoint {}".format(endpoint))
    ...
    def block_get(self, height):
        ...
        if not isinstance(height, int):
            raise TypeError("height has to be an int, invalid {} ({})".format(height, type(height)))
        if not height in self._blocks:
            raise tferrors.ExplorerNoContent("no content found for block {}".format(
    ...
    def block_add(self, height, resp, force=False):
        ...
        if not isinstance(resp, str):
            raise TypeError("resp has to be an str, invalid {} ({})".format(resp, type(resp)))
    ...
    def hash_get(self, hash):
        ...
        if not isinstance(hash, str):
            raise TypeError("hash has to be an str, invalid {} ({})".format(hash, type(hash)))
        if not hash in self._hashes:
            raise tferrors.ExplorerNoContent("no content found for hash {}".format(
                hash), endpoint="/explorer/hashes/{}".format(hash.__str__()))
    ...
    def mint_condition_add(self, condition, height, force=False):
        ...
        if height in self._mint_conditions and not force:
            raise KeyError("{} already exists in explorer mint conditions on height {}".format(
                condition.unlockhash.__str__(), height))
