...
export var ExplorerClient =  __class__ ('ExplorerClient', [object], {
	__module__: __name__,
	...
	get get_data () {return __get__ (this, function (self, endpoint) {
		...
		for (var idx of indices) {
			try {
				...
				var resp = jspolyfill.http_get (resource, __kwargtrans__ ({headers: headers}));
				if (resp.code == 200) {
	...
	get post_data () {return __get__ (this, function (self, endpoint, data) {
		var indices = list (range (len (self._addresses)));
		random.shuffle (indices);
		for (var idx of indices) {
			try {
				...
				var s = data;
				if (!(isinstance (s, str))) {
					var s = jspolyfill.json_dumps (s);
				}
				var resp = jspolyfill.http_post (resource, s, __kwargtrans__ ({headers: headers}));
				if (resp.code == 200) {
					return jspolyfill.json_loads (resp.data);
				}
...
Object.defineProperty (ExplorerClient, 'addresses', property.call (ExplorerClient, ExplorerClient._get_addresses));;
