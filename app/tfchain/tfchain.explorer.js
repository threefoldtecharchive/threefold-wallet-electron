	...
	get get_data () {return __get__ (this, function (self, endpoint) {
		...
		for (var idx of indices) {
			try {
				...
				var resp = jspolyfill.http_get (resource);
				if (resp.code == 200) {
					return resp.data;
				}
				if (resp.code == 204) {
				...
				var __except0__ = tferrors.ExplorerServerError ('error (code: {})'.format (resp.code), endpoint);
				__except0__.__cause__ = null;
				throw __except0__;
			}
			catch (__except0__) {
	...
	get post_data () {return __get__ (this, function (self, endpoint, data) {
		...
		for (var idx of indices) {
			try {
				...
				var resource = address + endpoint;
				var resp = jspolyfill.http_post (resource, data);
				if (resp.code == 200) {
					return resp.data;
				}
